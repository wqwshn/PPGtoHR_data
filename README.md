...
完整文档分类见 [文档导航](docs/README.md)。

## Windows 日常入口

双击根目录 **`start.bat`**，统一使用“原始数据采集”和“固件配置与烧录”。
旧 `tools/monitor/start_monitor.bat` 仍兼容；其他启动脚本归入 `scripts/`。
文件树、参数含义、接线和操作步骤见 [统一工作台使用说明](docs/统一工作台使用说明.md)。
下文部分双面板描述为早期设计；当前 UI 不再提供独立在线心率页面。

基于 STM32L452CEU6 的实时 PPG 光电心率监测系统，配套 PyQt5 上位机可视化软件。

## 当前交付状态

- 根目录 `start.bat` 统一启动采集与烧录工作台；旧启动入口兼容。
- 标准固件：Raw 100 Hz、35字节单帧发送、启动复位后写入 +2.5 dBm（无读回确认）。
- 热膜显示默认3点中值，端口旁“原始数据”按钮可切换；CSV同时保存原始与中值列。
- 固件页保留PPG/IIC、蓝牙初始化、光模式和ST-Link参数，实验模式不再显示。
- 全部实验过程、定量结果、限制与后续建议见 [热膜与蓝牙实验总结](docs/experiments/热膜与蓝牙实验总结.md)。
- 历史实验脚本保留用于复现；`recordings/`、`reports/`、`build/`及本机配置不提交。
