 ...
 * ============================================================
 * TODO: 以下系数为占位值, 需使用 MATLAB/Octave/Python scipy 精确计算.
 *       推荐使用:
 *         from scipy.signal import butter
 *         sos = butter(4, [0.5, 5], btype='bandpass', fs=125, output='sos')
 *       然后将 SOS 转换为 CMSIS-DSP biquad 格式 [b0, b1, b2, -a1, -a2].
 *       Task 8 将验证这些系数的正确性.
 *
 * 当前占位值基于 scipy.signal.butter(4, [0.5, 5], btype='bandpass', fs=125)
 * 的近似计算结果, 每节格式: [b0, b1, b2, -a1, -a2]
 */
const float HR_BPF_COEFFS[10] = {
    /* 第 1 节: [b0, b1, b2, -a1, -a2] */
    0.0050f,  0.0000f, -0.0050f,  1.7897f, -0.9428f,
    /* 第 2 节: [b0, b1, b2, -a1, -a2] */
    0.0050f,  0.0000f, -0.0050f,  1.7952f, -0.9689f
};
