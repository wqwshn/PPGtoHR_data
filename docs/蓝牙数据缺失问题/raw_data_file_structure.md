...
每次录制使用同一个时间戳前缀，常规生成两个 CSV；自动射频实验另生成事件 CSV：
...
- `InterpFlag`：当前为 `InterpFlag=0`，表示缺失行只占位，不做自动插值（No automatic interpolation）。
...
自动三轮固件发送事件时，同次录制额外生成 `raw_data_YYYYMMDD_HHMMSS_rf_events.csv`。含 MCU 时刻、采样计数、连接脚边沿、复位命令、轮次、阶段、队列丢失计数及1Hz快照。具体字段见自动三轮实验说明。常规固件不生成该文件。

例如连续缺失 30-50 个样本（100Hz 下为0.3-0.5秒），主CSV仍只填NaN，不自动补出测量值。
