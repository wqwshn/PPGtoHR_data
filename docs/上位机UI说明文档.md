...
- 启动直接显示原始数据，移除在线心率/原始数据切换按钮，不创建 HR 面板。Raw 页内实时 FFT 心率保留。
- 工具栏使用实际安装的 Microsoft YaHei UI / Microsoft YaHei / Segoe UI 字体，10pt 正文字号、38 逻辑像素控件高度和统一间距。主入口启用高 DPI 缩放，图表标题和坐标使用无衬线字体。
- 原始数据作为固定页面标题；连接和录制作为主要操作，其余按钮使用低饱和背景。保存路径按钮统一显示“选择保存路径”，完整目录显示在悬浮提示中，避免长目录挤压工具栏。
- 采样率、丢包率、心率与诊断分两行展示，允许换行。
- 清屏、录制只作用于 Raw 页面；清屏同步重置录制按钮，关闭窗口会停止模拟并刷盘关闭录制文件。
- `--simulate` 兼容为 `--raw-simulate` 别名，旧启动快捷方式仍可用，但不再打开 HR 页面。
