...
### 模块射频复位三轮（2026-09-15 恢复入口）

在“固件配置与烧录 → 蓝牙配置”选择“模块射频复位三轮 · 实际连接30秒 / 保持复位30秒”，然后点击“编译并烧录”。热膜噪声对照实验选择“不使用 PPG”，输出保持 Raw 100 Hz。该模式自动关闭蓝牙初始化，启用 `BLE_RF_EXPERIMENT=1`，不设置发射功率；标准采集选项仍保留。保存选择后，下次打开面板会恢复该模式。

烧录后有 60 秒准备期：打开有线 COM 并开始录制，HJ-380 全程插入，保持供电、姿态和防风条件一致。每轮实际连接 30 秒、模块保持复位 30 秒，释放后等待实际重连；共三轮。看到“三轮完成 / COMPLETE”后再录约 5 秒，保存同时间戳的主 CSV、`_status.csv` 和 `_rf_events.csv`，供后续按阶段分析。如果错过准备期和首段，复位 MCU 后重新完整录制。

详细状态规则和事件字段见[自动三轮射频实验](experiments/archive/自动三轮射频实验.md)。本次仅恢复 UI 入口，沿用现有固件状态机。
...
- 字体沿用远程 `Arial` / `SimSun` 字体族；工具栏为 10pt、30 逻辑像素高度，状态信息 11pt，启用高 DPI 缩放。
- 工具栏合为一行，依次放热膜显示切换、串口、连接/断开、刷新、清屏、保存路径、实验信息、Marker、录制、连接状态和语言切换。按钮间距 6px，垂直边距 4px；保存目录全文放在悬浮提示中。腾出的高度用于放大信息栏文字和扩展曲线，Ut:Uc 仍为 3:2。
- 状态区固定两行：第一行显示模式、采样率、丢包、计数及设备消息；第二行并排显示实时 FFT 心率、频谱 SNR（dB）、计算耗时和链路诊断。较长诊断或设备消息显示省略号，悬浮可查看全文，不增加信息栏高度。
- Ut1/Ut2 曲线行与 Uc1/Uc2 曲线行的高度比例为 3:2，压缩信息栏后增加 Ut 行的显示空间。
- Uc1/Uc2 为 ADC 桥中电压（mV），没有计算 `Ut / (Ut - Uc)` 比值。显示与 CSV 均使用原始电压，不做中值滤波。
- 保留本地 PPG nA 显示和曲线均值；CSV 的 PPG 原始 ADC 码值不变。
- 保留“实验信息”命名与按需 Marker，禁止覆盖同名文件。仅记录热膜原始值，另存 `_processing.json`（`filter=none`）；RF 事件按需写入 `_rf_events.csv`。
...
## 2026-09-14：单帧 +2.5 dBm 与热膜双数据录制（历史方案）
...
## 2026-09-24：恢复纯原始信号与24小时录制

蓝牙去耦电容修正后，取消热膜三点中值处理及显示切换。“原始数据”为固定标题；Uc1/Uc2/Ut1/Ut2直接显示接收值。新 CSV 仅保存原始通道与时间轴、质量字段，不再生成四列 Median3 或 Median3Valid；原始电压仍按既有规则保留5位小数。历史文件保持不变。

保留 `_processing.json` 作为录制说明，版本2、`filter=none`。缺帧仍按序号补 NaN、ValidFlag=0；`_status.csv` 与按需 Marker/RF 记录继续保留。辅助接收及事件时间使用单调时钟，原始样本时间仍为 SampleIndex/100，不受系统校时影响。

标准Raw 100 Hz模式下，24小时约864万行。计数器、缓存和序号回绕检查见[24小时采集检查](24小时原始数据采集检查.md)。该结论为代码审查及加速测试，不等同于真实设备24小时耐久验证。
