> 历史资料：合并前 README 的完整内容。部分参数、页面和路径已过时，当前使用请见[项目首页](../../../README.md)与[文档导航](../../README.md)。

# PPG 心率监测系统

完整文档分类见 [文档导航](../../README.md)。

## Windows 日常入口

双击根目录 **`start.bat`**，统一使用“原始数据采集”和“固件配置与烧录”。
旧 `tools/monitor/start_monitor.bat` 仍兼容；其他启动脚本归入 `scripts/`。
文件树、参数含义、接线和操作步骤见 [统一工作台使用说明](../../统一工作台使用说明.md)。
下文部分双面板描述为早期设计；当前 UI 不再提供独立在线心率页面。

基于 STM32L452CEU6 的实时 PPG 光电心率监测系统，配套 PyQt5 上位机可视化软件。

当前多光谱原始数据采集请先阅读 [多光谱原始数据采集核心功能指南](../../acquisition/多光谱原始数据采集核心功能指南.md)，包含单片机架构、UI 采集操作、传输协议及 CSV 保存格式。
## 当前交付状态
...
- 全部实验过程、定量结果、限制与后续建议见 [热膜与蓝牙实验总结](../../experiments/热膜与蓝牙实验总结.md)。
