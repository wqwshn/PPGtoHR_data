...
20 字节心率结果包 (1Hz, 蓝牙/串口 115200bps):
  偏移  字段                类型        说明
...
PAYLOAD_END = 18     # XOR 校验前一字节 (含)
XOR_START = 2        # XOR 计算起始偏移
XOR_END = 17         # XOR 计算结束偏移 (含)
...
def parse_hr_packet(data: bytes) -> Optional[HRPacket]:
    ...
    # 校验帧尾
    if data[19] != FOOTER_BYTE:
        return None

    # XOR 校验: bytes[2..17] 异或 == data[18]
    xor_val = 0
